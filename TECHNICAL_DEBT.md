...
### CH3-BUG-02 FIX (Chapter 3)
`isClosingNow` добавлен в условие `needsMountingSnapshot` в `CalculationClient.tsx`.
Теперь snapshot создаётся при любом первичном закрытии заказа, даже без предшествующего price lock.

### Приоритет
✅ ВЫПОЛНЕНО. price lock + done/cancelled оба покрыты.
...
### ✅ ЗАКРЫТО — CH2-BUG-01 (Chapter 2)
Оба slug добавлены в `defaultPrices.ts`. `addo_strap_cost` помечен [устарел].

### Приоритет
✅ ВЫПОЛНЕНО в Chapter 2.
---
...
### ✅ ЗАКРЫТО — CH3-BUG-02 (Chapter 3)
`isClosingNow` добавлен как условие в `needsMountingSnapshot` в `CalculationClient.tsx`.
При переводе в done/cancelled (если монтаж включён, snapshot отсутствует):
`captureCurrentPriceSnapshot()` вызывается автоматически перед сохранением.

### Приоритет
✅ ВЫПОЛНЕНО в Chapter 3.


---

## CHAPTER 3 — MEASUREMENT / MOUNTING / ANALYTICS

---

## 12. MEASUREMENT — TYPE SAFETY

### Состояние
`ClientFormData.measurementDate` и `installDate` типизированы как `any`.
`formatDateInputValue` теперь обрабатывает все форматы корректно (CH3-BUG-01 fix).
Но тип `any` в интерфейсе остаётся техническим долгом.

### Решение (будущее)
```typescript
measurementDate?: string | null;  // всегда "YYYY-MM-DD" или null
installDate?:     string | null;
```
Привести всю цепочку: Prisma → page.tsx → ClientFormData → handleChange → payload.

### Приоритет
Низкий. Функционально исправлено. Типы — при следующем рефакторинге интерфейсов.

---

## 13. ANALYTICS — orderFinancials (УДАЛЕНО)

### Состояние
В `ClientStep.tsx` существовал мёртвый `useMemo orderFinancials`, который использовал
`currentPrices` напрямую (нарушение price isolation для frozen-заказов).
Переменная никогда не рендерилась — удалена в CH3-BUG-03 (Chapter 3).

### Приоритет
✅ ЗАКРЫТО. Удалено безопасно.

---

## 14. ANALYTICS — FULL LAYER EXPORT

### Состояние
`useCalculationState` экспортирует отдельные компоненты:
`windowsRetailTotal`, `windowsExpensesTotal`, `extrasRetailTotal`, `extrasCostTotal`,
`mountingRetailTotal`, `mountingCostTotal`.

### Нереализовано
Агрегированный аналитический API по организации:
- суммы по периодам (месяц/квартал)
- breakdown по material type (PVC/TPU/TINTED/MOSQUITO)
- эффективность бригад

### Приоритет
Средний. После стабилизации Chapter 1–3.